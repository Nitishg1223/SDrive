...
class App extends Component {
  ...
  onDrop = async(file) =>{
    try {
      ...
      const generated_hash=result.cid.string;
      console.log(generated_hash);
      const timestamp= Math.round(+new Date()/1000); 
      const type=file.name.substr(file.name.lastIndexOf(".")+1);
      let uploaded=await contract.methods.add(generated_hash,file.name,type,timestamp).send({from:accounts[0],gas:300000});
           ...
           <Table>
             <thead>
               <tr>
                 ...
                 <th className="px-3 " style={{textAlign:"left"}} >File Name</th>
             ...
             <tbody>
               ...
               <th ><FileIcon  extension={item[2]} {...defaultStyles[item[2]]}/></th>
                 <th className="px-3" style={{textAlign:"left"}} ><a href={"https://ipfs.io/ipfs/"+item[0]} target="_blank"> {item[1]}</a></th>
